...
VARorder(da_inflation, maxp = 24) # order 17
...
da_trade_oil <- da %>%
  ...
  mutate(imp_china = log10(imp_china), imp_all = log10(imp_all)) %>%
  drop_na() %>%
  select(imp_all, imp_china, oil) 
  

VARXorder(x = da_ppi_cpi, exog = da_trade_oil, maxp = 12, maxm = 3)

m23 <- VARX(zt = da_ppi_cpi, p = 2, xt = da_trade_oil, m = 3)
...
```{r report china tw hk trade, include=TRUE, cache=TRUE}
...
m10 <- refECMvar1(m9, thres = 1.2)
m12 <- refVAR(m11, thres = 1.2) # no cointergration the test is on borderline
...
```{r report cpi import china oil, include=TRUE, cache=TRUE}

ca.jo(da_inflation_china, ecdet = "const", type = "trace", K = 14, spec = "transitory") %>%
  summary()

m16 <- refECMvar1(m15, thres = 1.2)
m22 <- refVAR(m21, thres = 1.2)
m20 <- refsVARMA(m19, thres = 1.2)

```

Use trade and oil as exogenous shock to model the cpi and ppi.

```{r report exogenous shock, include=TRUE, cache=TRUE}
